...
function download_hadoop(){
    ...
    if which hadoop &>/dev/null; then
        ...
        read -p "Have you already downloaded the tar file ? y/n : " opt
        if [ $opt == 'y' ]; then
            read -p "Enter path of tar file : " user_path
           if [[ "$user_path" = "~" || "$user_path" = "/" || ( -d "$user_path" && -w "$user_path" ) ]]; then
                tar -xzvf $user_path"/"$hadoopVersion.tar.gz
            else
                echo "Path does't exists"
                download_hadoop $hadoopVersion
            fi
